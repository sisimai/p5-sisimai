...
- changes:
  - Fix issue #491, Remove X-Auto-Response-Suppress: header from the inspection
    ...
    Thanks to @whity
  - Fix issue #492, To avoid slowing down the parsing speed, reduce the size of
    the email by removing unnecessary parts (`text/html`, `application/*`, and
    `image/*`). Thanks to @gody01
...
- changes:
  ...
  - Import sisimai/rb-sisimai#244, Prevent ReDOS in `Sisimai::String->to_plain`
    method #484
...
- changes:
  ...
  - #421 Fixed a serious bug at the value of `token` in the parsed results only
    ...
    value of `token` generated by Sisimai::Data->new method between v4.22.1p1 to
