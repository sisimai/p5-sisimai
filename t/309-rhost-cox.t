...
my $PackageName = 'Sisimai::Rhost::Cox';
my $MethodNames = {
    'class' => ['get'],
    'object' => [],
};
...
MAKE_TEST: {
    my $rs = {
        '01' => { 'status' => qr/\A5[.]1[.]0\z/, 'reason' => qr/blocked/ },
    };
    ...
    PARSE_EACH_MAIL: for my $n ( keys %$rs ) {
        my $emailfn = sprintf("./set-of-emails/maildir/bsd/rhost-cox-%02d.eml", $n);
        my $mailbox = Sisimai::Mail->new($emailfn);
        my $mtahost = qr/cox[.]net\z/;
        ...
        while( my $r = $mailbox->data->read ) {
            ...
            for my $e ( @{ $p->ds } ) {
                ...
                like $e->{'command'}, qr/\A(?:MAIL|)\z/, '->command = '.$e->{'command'};
