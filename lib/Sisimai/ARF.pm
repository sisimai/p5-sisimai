...
sub is_arf {
    ...
    if( $argvs->{'content-type'} =~ $RxARF0->{'content-type'} ) {
        # Content-Type: multipart/report; report-type=feedback-report; ...
        $match = 1;

    } elsif( $argvs->{'content-type'} =~ $RxARF1->{'content-type'} ) {
        # Microsoft (Hotmail, MSN, Live, Outlook) uses its own report format.
        # Amazon SES Complaints bounces
        if( $argvs->{'from'} =~ $RxARF1->{'from'} && $argvs->{'subject'} =~ $RxARF1->{'subject'} ) {
...
sub scan {
    ...
    my $dscontents = [];    # [Array] SMTP session errors: message/delivery-status
    my $rfc822head = undef; # [Array] Required header list in message/rfc822 part
    ...
    #
    for my $e ( @stripedtxt ) {
        # Read each line between $RxARF0->{'begin'} and $RxARF0->{'rfc822'}.
        unless( $readcursor ) {
            # Beginning of the bounce message or delivery status part
            $readcursor = $indicators->{'deliverystatus'} if $e =~ $RxARF0->{'begin'};
        }

        unless( $readcursor & $indicators->{'message-rfc822'} ) {
            # Beginning of the original message part
            $readcursor = $indicators->{'message-rfc822'} if $e =~ $RxARF0->{'rfc822'};
        }
