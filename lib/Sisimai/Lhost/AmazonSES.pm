...
my $StartingOf = { 'message' => ['The following message to <', 'An error occurred while trying to deliver the mail '] };
...
sub make {
    ...
    if( index($$mbody, '{') == 0 ) {
        ...
        return undef unless $mhead->{'from'} =~ /MAILER-DAEMON[@](?:[0-9A-Za-z-]+[.])?amazonses[.]com/;

        my $match = 0;
        $match++ if $mhead->{'x-aws-outgoing'};
