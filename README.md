...
bounceHammer 4, is the successor of bounceHammer.
...
Callback feature
-------------------------------------------------------------------------------
`c___` argument of `Sisimai->make` and `Sisimai->dump` is an array reference and
is a parameter to receive code references for callback feature. The first element
of `c___` is called at `Sisimai::Message->parse` for dealing email headers and entire
message body. The result generated by the callback method is accessible via `Sisimai::Data->catch`.
...
Callback method set in the second element of `c___` is called at `Sisimai->make()`
method for dealing each email file.
